...
De plus, il est utile de comprendre que les fonctions d'activation choisies auront la propriété utile de permettre en temps réduit le calcul de leur dérivée en fonction de leur résultat : on a pour tout $x$, $f'(x)=G(f(x))$ , ce qui va permettre, en retenant les résultats de chaque neurone pour la propagation directe, d'effectuer rapidement la propagation indirecte. 

Enfin, le processus de minimisation se fait par des méthodes basées sur la méthode du gradient vectoriel:

$$Sys(n+1) = Sys(n) - Ereur[Sys(n)] \times Jacobienne(Erreur[Sys(n)]) ^{-1} $$ 

L'algorithme va donc dépendre, en plus du choix de la fonction de coût et de la fonction d'activation des neurones, du choix de la méthode de minimisation et de ses paramètres associés (learning rate).
...
\subsection{Application au problème de classification de chiffres manuscrits}
...
\subsection{Principes des CNN - Convolutional Neuronal Networks}
...
Une fois l'étape des CNN terminée, les professeurss encadrants nous ont donné pour but de travailler sur le Q-learning : une théorie de l'apprentissage par machine tout à fait différente des perceptrons, avec des cas d'applications tout aussi différents.
...
\begin{figure}
\subfloat[état initial du plateau]{\includegraphics[width=0.49 \linewidth]{Qlearning.png}}
\subfloat[exploration du plateau]{\includegraphics[width=0.49 \linewidth]{Qlearning2.png}}
\newline
\subfloat[mise à jour des valeurs du tableau]{\includegraphics[width=0.49 \linewidth]{Qlearning3.png}}
\subfloat[exploitation après apprentissage]{\includegraphics[width=0.49 \linewidth]{Qlearning4.png}}
\centering
...
\begin{itemize}
	\item L'utilisation d'un réseau de neurones est absolument nécessaire : utiliser simplement l'observation comme indexeur d'états donnerait un ensemble d'états possibles bien trop grand (position de la balle, des joueurs, changement du score, etc...)
...
\begin{figure}
\includegraphics[width=0.49\linewidth]{pong-filter.png}
\centering
...
\begin{figure}
\includegraphics[width=\linewidth]{pong-plateau.png}
\centering
\caption{Plateau de jeu de Pong et features associées}
\label{fig:features}
\end{figure}
...
Néanmoins, au vu des difficultés à faire fonctionner notre programme pour Pong, nous avons décidé d'apporter un léger pré-traitement :
\begin{itemize}
	\item Supprimer les images inutiles qui n'influencent pas le déroulement du jeu. Cela permet de moins apprendre sur des actions qui n'ont pas de réel impact sur la suite de la partie.
...
Pour remédier à ce problème, nous avons décidé de modifier la fonction de récompense du jeu. Nous avons en effet ajouté une récompense dite "de survie", plus faible que la récompense de victoire, mais qui arrive beaucoup plus fréquemment. Nous encouragions donc notre programme à apprendre les coups qui lui permettaient de ne pas perdre, c'est à dire de renvoyer la balle correctement. Cette méthode est inspirée du jeu de voiture présenté en figure \ref{fig:inter-rewards}, où l'intelligence artificielle doit apprendre à réaliser des tours de circuit, en étant récompensée à chaque porte.

\begin{figure}
\includegraphics[width=\linewidth]{inter-rewards.png}
\centering
\caption{Récompenses intermédiaires du jeu de voiture}
\label{fig:inter-rewards}
\end{figure}
...
Nous avons aussi pu tester l'influence du dropout sur ces résultats (voir figure \ref{fig:dropout-autoencodeur}). Etonnement, les meilleurs résultats sont obtenus avec un dropout de 90\%. On peut voir que les nombres sont assez fidèlement reproduits avec seulement 16 features, comme le montre la figure \ref{fig:autoencodeur}.
...
\begin{figure}
\subfloat[Autoencodage d'un 0]{\includegraphics[width=0.5\linewidth]{auto0.png}}
\subfloat[Autoencodage d'un 3]{\includegraphics[width=0.5\linewidth]{auto3.png}}
\newline
\subfloat[Autoencodage d'un 4]{\includegraphics[width=0.5\linewidth]{auto4.png}}
\subfloat[Autoencodage d'un 5]{\includegraphics[width=0.5\linewidth]{auto5.png}}
\newline
\subfloat[Autoencodage d'un 8]{\includegraphics[width=0.5\linewidth]{auto8.png}}
\centering
\caption{Autoencodage de différents nombres du set de données MNIST avec 16 features sous TensorFlow}
\label{fig:autoencodeur}
\end{figure}
...
\begin{figure}
\includegraphics[width=\linewidth]{flappy-bird-plateau.jpg}
\centering
\caption{Présentation du plateau de Flappy Bird}
\label{fig:flappy-bird-plateau}
\end{figure}
...
Un pendule retourné attaché à un chariot doit rester en équilibre le plus longtemps possible. Le réseau a deux actions possibles : diriger le chariot vers la gauche ou vers la droite (voir figure \ref{fig:cartpole-plateau}). Encore une fois, le réseau dispose de features associées à l’état du chariot.
\begin{figure}
\includegraphics[width=\linewidth]{cartpole-plateau.png}
\centering
\caption{Présentation du plateau de jeu CartPole}
\label{fig:cartpole-plateau}
\end{figure}
...
\newpage
\renewcommand{\refname}{\spacedlowsmallcaps{Références}} % For modifying the bibliography heading
\nocite{*}
\bibliographystyle{plain}
\bibliography{ready-player-one-supelec}
\end{document}
